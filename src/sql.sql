...
alter table  agencies add constraint phone_number check(phone_number ~ '^\+996\d{9}$');
...
select o.owner, h.house from owners o join houses h on o.owner_id =h.owner_id where length (o.owner)=(select max(length(owner))from owners);
select exists(select 1 from houses where price>=1500 and price <=2000 );
